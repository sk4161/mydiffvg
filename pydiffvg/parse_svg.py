...
def parse_hex(s):
    ...
    return torch.pow(
        torch.tensor([rgb[0] / 255.0, rgb[1] / 255.0, rgb[2] / 255.0]), 1.0
    )
...
def parse_color(s, defs):
    ...
    if s[0] == "#":
        ...
        rgb = s[4:-1].split(",")
        color = torch.tensor(
            [int(rgb[0]) / 255.0, int(rgb[1]) / 255.0, int(rgb[2]) / 255.0, 1.0]
        )
    elif s == "none":
        return None
    else:
...
def parse_common_attrib(node, transform, fill_color, defs):
    ...
    if "style" in attribs:
        ...
        if isinstance(fill_color, pydiffvg.LinearGradient):
            fill_color.begin = new_transform @ torch.cat(
                (fill_color.begin, torch.ones([1]))
            )
            fill_color.begin = fill_color.begin / fill_color.begin[2]
            fill_color.begin = fill_color.begin[:2]
            fill_color.end = new_transform @ torch.cat(
                (fill_color.end, torch.ones([1]))
            )
        ...
        if isinstance(stroke_color, pydiffvg.LinearGradient):
            stroke_color.begin = new_transform @ torch.cat(
                (stroke_color.begin, torch.ones([1]))
            )
            stroke_color.begin = stroke_color.begin / stroke_color.begin[2]
            stroke_color.begin = stroke_color.begin[:2]
            stroke_color.end = new_transform @ torch.cat(
                (stroke_color.end, torch.ones([1]))
            )
